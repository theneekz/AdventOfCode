...
// Loop through in groups of three
for (let i = 2; i < inputArr.length; i += 3) {
  const rucksackA = inputArr[i]; // index 2
  const rucksackB = inputArr[i - 1]; // index 1
  const rucksackC = inputArr[i - 2]; // index 0
  // Find similarity by splitting string into an array and filtering
  const similarity = rucksackA
    .split("")
    .filter((el) => rucksackB.indexOf(el) != -1 && rucksackC.indexOf(el) != -1);
  priorities.push(similarity[0]);
}
