  ...
  for (const line of input) {
    ...
    for (let i = Number(amount); i > 0; i--) {
      ...
      for (let i = 0; i < rope.length - 1; i++) {
        const [headKnot, tailKnot] = [rope[i], rope[i + 1]];
        const [headX, headY] = headKnot;
        const [tailX, tailY] = tailKnot;
        // If the knot following this one is not touching it
        if (Math.abs(headX - tailX) > 1 || Math.abs(headY - tailY) > 1) {
          // Update following knot of rope
          rope[i + 1] = moveKnot(tailKnot, headKnot);
          const isLastKnot = i + 1 === 9;
          if (isLastKnot) {
            // Either update # of visits to this coord or initialize count to 1
            visited[convert(rope[i + 1])] =
              visited[convert(rope[i + 1])]++ || 1;
          }
        } else {
          // Don't need to check the following knots if one doesn't move
          break;
        }
      }
    }
...
function moveKnot(oldTail, newHead) {
