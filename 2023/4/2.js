...
function main() {
  ...
  let results = Array.from({ length: input.length }).fill(1);
  for (let i = 0; i < input.length; i++) {
    const line = input[i];
    const [unused, rest] = line.split(": ");
    let [winners, lottery] = rest.split(" | ");
    winners = winners.split(" ").filter((x) => x);
    lottery = lottery.split(" ").filter((x) => x);
    let matches = 0;
    for (const winner of winners) {
      if (lottery.indexOf(winner) !== -1) {
        matches++;
      }
    }
    if (matches) {
      updateCards(results, i, matches);
    }
  }
  return results.reduce((p, c) => p + c, 0);
}

const updateCards = (a, i, matches) => {
