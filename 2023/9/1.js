...
function main() {
  ...
  for (const line of input) {
    const pattern = findNextInPattern(line).at(-1);
    missingValues.push(line.at(-1) + pattern);
  }
  return missingValues.reduce((a, b) => a + b, 0);
}

const findNextInPattern = (arr) => {
  ...
  const diffOfDiffs = findNextInPattern(diffs);
  const last = diffs.at(-1) + diffOfDiffs.at(-1);
